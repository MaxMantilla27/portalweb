...
import {
  ChatAtencionClienteContactoActualizarDTO,
...
export class ChatAtencionVentasComponent implements OnInit, OnChanges {
            ...
            if (this.Paso == 2 && this.Caso == 'A') {
              this._ChatAtencionClienteService
                ...
                .subscribe({
                  ...
                  complete: () => {
                    this.CargandoInformacion = false;
                    let AreaSeleccionado = this.AreasCapacitacion.filter(
                      (areas: any) =>
                        areas.valor ===
                        this.RegistroHistoricoUsuario.mensajeEnviado
                    );
                    console.log(AreaSeleccionado);
                    this.CursosPorArea = [];
                    this._ChatAtencionClienteService
                      .ListaProgramasFiltroChatAtc(AreaSeleccionado[0].id)
                      .pipe(takeUntil(this.signal$))
                      .subscribe({
                        next: (x) => {
                          console.log(x);
  ...
  RecuperarContrasena(evento: any) {
    ...
    this._AccountService
      .RecuperarPasswordCuenta(evento.email)
      .pipe(takeUntil(this.signal$))
      .subscribe({
        next: (x) => {
          console.log(x);
        ...
        complete: () => {
          ...
          this.RegistroChatDetalleAtc.CasoSiguiente = 'B';
          this.RegistroChatDetalleAtc.MensajeEnviado =
            'Recuperar Contraseña Correo:' + evento.email;
          this._ChatAtencionClienteService
            .RegistrarChatAtencionClienteContactoDetalle(
              this.RegistroChatDetalleAtc
            )
  ...
  CursoPadreHijoMatriculado(courseId: number): void {
    ...
    this.selectedPadreIdMatricula =
      this.selectedPadreIdMatricula === courseId ? null : courseId;
  }
  getCursosHijo(courseId: number): any[] {
    return this.CursosHijoMatricula.filter(
      (hijo: any) => hijo.idPGeneralPadre === courseId
    );
  }
  handleCursoClickFormulario(curso: any): void {
    if (curso.esPadre) {
      this.selectedPadreIdMatricula =
        this.selectedPadreIdMatricula === curso.idPGeneralPadre
          ? null
          : curso.idPGeneralPadre;
    } else {
        ...
        .subscribe({
          ...
          complete: () => {
            (this.ActualizarChatAtc.Id = this.IdChatAtencionClienteContacto),
              ...
              (this.ActualizarChatAtc.IdFaseOportunidadPortal =
                '00000000-0000-0000-0000-000000000000'),
              (this.ActualizarChatAtc.IdMatriculaCabecera =
                CursoHijo.idMatriculaCabecera),
              (this.IdMatriculaCabecera = CursoHijo.idMatriculaCabecera),
              (this.IdProgramageneral = CursoHijo.idPGeneralPadre);
            this._ChatAtencionClienteService
              .ActualizarChatAtencionClienteContacto(this.ActualizarChatAtc)
              .pipe(takeUntil(this.signal$))
              .subscribe({
                next: (x) => {},
                complete: () => {
  ...
  ObtenerCursosIdArea(Area: any) {
    ...
    this.CursosPorArea = [];
    this._ChatAtencionClienteService
      .ListaProgramasFiltroChatAtc(Area.id)
      .pipe(takeUntil(this.signal$))
      .subscribe({
        next: (x) => {
          this.CursosPorArea = x;
  ...
  RegistrarCursoSeleccionado(CursoSeleccionado: any) {
    ...
    this._SeccionProgramaService
      .ObtenerCabeceraProgramaGeneral(CursoSeleccionado.idBusqueda)
      .pipe(takeUntil(this.signal$))
      .subscribe({
        next: (x) => {
          this.IdPespecificoPrograma =
            x.programaCabeceraDetalleDTO.listProgramaEspecificoInformacionDTO[0].id;
        },
        complete: () => {
          (this.ActualizarChatAtc.Id = this.IdChatAtencionClienteContacto),
            (this.ActualizarChatAtc.IdPGeneral = CursoSeleccionado.idPGeneral),
            (this.ActualizarChatAtc.IdPEspecifico = this.IdPespecificoPrograma),
  ...
  RetrocederAreaA() {
    ...
    this.RegistroChatDetalleAtc.CasoSiguiente = 'A';
    this.RegistroChatDetalleAtc.MensajeEnviado =
      'Volver de Curso a Área de Capacitación';
    this._ChatAtencionClienteService
      .RegistrarChatAtencionClienteContactoDetalle(this.RegistroChatDetalleAtc)
      .pipe(takeUntil(this.signal$))
      .subscribe({
  ...
  ActualizarEsSoporteTecnico(EsSoporteTecnico: boolean) {
    ...
    if (EsSoporteTecnico == false) {
      ...
      this.RegistroChatDetalleAtc.MensajeEnviado =
        'Tengo problemas tecnicos en el aula virtual';
    }
    this._ChatAtencionClienteService
      .RegistrarChatAtencionClienteContactoDetalle(this.RegistroChatDetalleAtc)
      .pipe(takeUntil(this.signal$))
      .subscribe({
        ...
        complete: () => {
          this._ChatAtencionClienteService
            .ActualizarEsSporteTecnicoChatAtencionClienteContacto(
              this.IdChatAtencionClienteContacto,
              EsSoporteTecnico
            )
            .pipe(takeUntil(this.signal$))
  ...
  RegistroDirectoCursoMatriculado(EsSoporteTecnico: boolean) {
    ...
    if (this.IdChatAtencionClienteContacto == 0) {
      this._DatosPerfilService
        .RegistroProgramaMatriculadoPorIdMatricula(this.IdMatriculaCabecera)
        .pipe(takeUntil(this.signal$))
        .subscribe({
          next: (x) => {
            console.log(x);
  ...
  CerrarInteraccionFormularioAcademico() {
    this._ChatAtencionClienteService
      .CerrarFormularioAcademicoIdMatriculaCabecera(
        this.IdChatAtencionClienteContacto
      )
      .pipe(takeUntil(this.signal$))
      .subscribe({
