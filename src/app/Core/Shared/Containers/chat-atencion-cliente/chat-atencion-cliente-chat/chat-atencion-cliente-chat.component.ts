...
export class ChatAtencionClienteChatComponent
  implements OnInit, OnDestroy, OnChanges
{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    console.log('IdChatAtencionClienteContacto',this.IdChatAtencionClienteContacto)
  ...
  actualizarDatosAlumno(respuesta: any) {
    ...
    this.hubConnection
      .invoke('ActualizarDatosAlumno', respuesta.idAlumno, respuesta.id)
      .then((response: any) => {
        // Maneja la respuesta aquí
        console.log(
  ...
  configuracion() {
    this.hubConnection.on('configuracion', (NombreAsesor: any, estado: any) => {
      ...
      this.img +=
        this.nombreAsesorSplit[0] + '-' + this.nombreAsesorSplit[1] + '.png';
      this.ChargeChat.emit(true);
    });
  }
  onlineStatus() {
          ...
          if (this.mensajesAnteriorePrevio != null) {
            if (this.mensajesAnteriorePrevio.length > 0) {
              timer(2000)
                .pipe(takeUntil(this.signal$))
                .subscribe((_) => {
                  this.RespuestaConexion.id =
                    this.mensajesAnteriorePrevio[0].idFaseOportunidadPortalWeb;
                  this.RespuestaConexion.idAlumno =
                    this.mensajesAnteriorePrevio[0].idAlumno;
  ...
  VerificarChatFinalizado() {
    ...
    this.hubConnection.on(
      'setFinalizarChatComercial',
      (IdOportunidad: number, IdAlumno: number, IdPGeneral: number) => {
        console.log('INGRESARÁ setFinalizarChatComercial');
