...
export class ChatEnLineaService {
  ...
  public ObtenerAsesorChat(IdPGeneral: number): Observable<any> {
    if (this.isBrowser) {
      return this.http.get<any>(
        this.urlBase + '/ObtenerAsesorChat?IdPGeneral=' + IdPGeneral
      );
    } else {
      return EMPTY;
    }
  }
  public ValidarCrearOportunidadChat(
    Json: ValidacionChatEnvioDTO
  ): Observable<any> {
    if (this.isBrowser) {
      console.log(Json);
      return this.http.post<any>(
        this.urlBase + '/ValidarCrearOportunidadChat',
        Json
      );
    } else {
      return EMPTY;
  ...
  public ProcesarAsignacionAutomaticaNuevoPortal(Id: string): Observable<any> {
    if (this.isBrowser) {
      console.log(Id);
      return this.http.get<any>(
        this.urlBase + '/ProcesarAsignacionAutomaticaNuevoPortal?Id=' + Id
      );
    } else {
      return EMPTY;
    }
