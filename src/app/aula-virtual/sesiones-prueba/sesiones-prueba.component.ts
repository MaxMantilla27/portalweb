...
export class SesionesPruebaComponent implements OnInit,OnDestroy {
  ...
  ObtenerEstructuraEspecificaCurso() {
    this._ProgramaContenidoService
      .ObtenerEstructuraEspecificaCursoAccesoPrueba(this.json).pipe(takeUntil(this.signal$))
      .subscribe({
        next: (x) => {
  ...
  mensajeDisable(capitulo:number){
    if(capitulo==1){
      this._SnackBarServiceService.openSnackBar('Debes completar los video anteriores para continuar','x',10,'snackbarCrucigramaerror');
    }
    else{
