...
export class ModuloSesionesOnlineComponent
  implements OnInit, OnChanges, OnDestroy
{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.videos != null && this.videos.length > 0) {
      this.videos.forEach((v: any) => {
        v.idFrameVideo =
          '<iframe (click)="Onplay(this)" src="https://player.vimeo.com/video/' +
          v.idVideo +
        ...
        next: (x) => {
          ...
          if (x != null) {
            x.forEach((d: any) => {
              console.log(d);
              d.disabled = false;
              // let HoraWebexOriginalInicial=moment.tz(d.fechaActual, this.ZonaHorariaOrigenWebex);
              let HoraWebexOriginal1: any;
              let HoraWebexOriginal2: any;
              HoraWebexOriginal1 = moment.tz(
                d.fechaEntrega,
                this.ZonaHorariaOrigenWebex
              );
              if (d.fechaEntregaSecundaria != null) {
                HoraWebexOriginal2 = moment.tz(
                  d.fechaEntregaSecundaria,
                  this.ZonaHorariaOrigenWebex
                );
                console.log(
                  'Hora Webex Original2:',
                  HoraWebexOriginal2.format('YYYY-MM-DD HH:mm:ss')
                );
              }
              ...
              console.log(
                'Hora Webex Original1:',
                HoraWebexOriginal1.format('YYYY-MM-DD HH:mm:ss')
              );
              console.log(
                'Hora Webex Original:',
              ...
              if (
                HoraWebexOriginal1.toString() === HoraWebexOriginal.toString()
              ) {
                d.calificacionActual = d.calificacionMaxima;
              } else {
                d.calificacionActual = d.calificacionMaximaSecundaria;
              }
          ...
          if (
            ...
            const dialogRef = this.dialog.open(EnvioTareaComponent, {
              ...
              data: {
                ...
                codigoIso: this.CodigoIsoPaisWebex,
              },
  ...
  EnviarCuestionario(indexSesion: number, index: number) {
    ...
    //         width: '1000px',
    //         data: {cuestionario:this.sesiones[indexSesion].cuestionario[index],index:index,IdMatriculaCabecera:this.IdMatriculaCabecera,codigoIso:this.CodigoIsoPaisWebex},
  ...
  EnviarActividad(indexSesion: number, index: number) {
    const dialogRef = this.dialog.open(EnvioActividadComponent, {
      width: '1000px',
      data: {
        actividad: this.sesiones[indexSesion].actividades[index],
        index: index,
        IdMatriculaCabecera: this.IdMatriculaCabecera,
      },
